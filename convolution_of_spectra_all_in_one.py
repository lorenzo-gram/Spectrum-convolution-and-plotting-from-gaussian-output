...
parser.add_argument("-t", "--transition", default='abs',  metavar='TRANS', dest="trans",
                    type=str, help="absorption (abs) or emission (emi)")
parser.add_argument("-n", "--nstates", default=-1,  metavar='NSTATES', dest="nstates",
                    type=int, help="The number of transitions to put in the legend")
...
plt.plot(spectral_grid_nm, spectrum/maximum, color='black', linewidth=2)
...
for i in range(n_lines):
    for j in range(n_sets):
        x = wavelength[j][i] 
        if args.trans=='abs':
            y = f[j][i]*n_sets/maximum 
            plt.vlines(x, 0, y, color=colors[i], label=rf'$S0 \rightarrow S{{{i+1}}}$' if j == 0 else "")
        elif args.trans=='emi':
            y = f[j][i]*n_sets*((1239.8/wavelength[j][i])**2)/maximum 
            plt.vlines(x, 0, y, color=colors[i], label=rf'$S{{{i+1}}} \rightarrow S0$' if j == 0 else "")
